...
for n_row, data_row in test_set.iterrows():
    if np.isnan(data_row["Longitud_tarso"]):
        morphometric_variable = "Masa"
        mass_train = data_row[morphometric_variable]
        interval = 1
    else:
        morphometric_variable = "Longitud_tarso"
        mass_train = data_row[morphometric_variable]
...
results_df.to_csv("pollos_petrel/datletas_encobijados_second_submission.csv", index=False)
